...
# KRX regular index review (June / December)
for y, m in [(2026, 12), (2027, 6)]:
    exp = nth_weekday(y, m, 3, 2)
    eff = next_kr_bday(exp)
    add(eff, "KR", "index", "KRX 정기변경 적용 — 코스피200·코스닥150 구성종목 변경", imp=3,
        desc="코스피200·코스닥150 정기변경이 적용되는 날(선물 만기일 다음 거래일). 전일 종가에 패시브 리밸런싱이 "
             "집중되고, 편입 예상 종목은 발표일부터 적용일까지 선반영되는 경향이 있습니다.",
        checklist=["편입·편출 확정 종목과 지수 추종자금 규모 확인",
                   "예상 수급(추종자금×비중변화) 대비 거래대금 배수 점검",
                   "선반영 여부 확인 후 적용일 차익 실현/신규 진입 판단"],
        links=[{"label": "지수 정기변경 트래커", "url": "https://bluelagoon1222.github.io/Index-Rebalance-Korea/"}],
        source="KRX 지수 정기변경 규정")
    add(exp, "KR", "index", "KRX 정기변경 리밸런싱 (만기일 종가)", imp=3,
        desc="정기변경 반영을 위한 패시브 리밸런싱이 만기일 종가에 실행됩니다.",
        source="KRX")
